    ...
    <div className="flex min-h-screen pt-14">
      <main id="content" role="main" className="flex-1 min-h-screen p-3 sm:p-4 xl:p-6 max-w-full overflow-hidden">
        <div className="mb-4 sm:mb-6">
          <button className="inline-flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg border border-gray-200 dark:border-gray-800 hover:bg-black/5 dark:hover:bg-white/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/40 mb-4" onClick={() => router.push("/")}>
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div className="flex-1 min-w-0">
                <input className="w-full text-lg sm:text-xl xl:text-2xl font-semibold bg-transparent border-b border-transparent focus:border-brand-500/40 focus:outline-none" value={name} onChange={(e) => setName(e.target.value)} />
                <div className="text-xs text-gray-500 mt-1">Style: {item.style}</div>
              </div>

              <div className="flex flex-col sm:flex-row gap-2 shrink-0">
                <select className="px-3 py-1.5 text-sm rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/40" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}>
                  {categories.map((c) => (
                ...
                <div className="flex gap-2">
                  <button
                    className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-3 py-1.5 text-sm rounded-lg bg-brand-600 text-white hover:bg-brand-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-brand-600"
                    onClick={save}
                    disabled={isLoading}
                  >
                    {isLoading && (
        ...
        {/* On mobile/tablet: show IconUploader in main; on desktop it's in the right sidebar */}
        <div className="xl:hidden mt-2">
          <IconUploader label="Icon/Thumbnail" value={previewThumbUrl || ""} onChange={setPreviewThumbUrl} />
        </div>

        {/* Preview tetap tampil di main untuk semua ukuran */}
